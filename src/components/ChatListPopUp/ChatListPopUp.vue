<template>
  <div class="robin-chat-list-popup robin-zoomIn">
    ...
    <div
      class="robin-wrapper robin-w-100"
      @click="$emit('archive-chat')"
      v-if="isArchiveChatEnabled && !isArchived"
    >
      <message-content :font-size="14" :color="currentTheme === 'light' ? '#101010' : '#F9F9F9'"
        >Archive Chat</message-content
      >
      <svg-icon name="mailbox" :color="currentTheme === 'light' ? '#51545C' : '#F9F9F9'" />
    </div>
    <div class="robin-wrapper robin-w-100" v-if="isArchived" @click="$emit('unarchive-chat')">
      <message-content :font-size="14" :color="currentTheme === 'light' ? '#101010' : '#F9F9F9'"
    ...
    <div
      ...
      v-if="isDeleteMessagesEnabled && !isArchived"
    >
