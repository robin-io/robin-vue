  ...
  <header>
      ...
      <div class="robin-flex robin-flex-align-center robin-clickable" @click="openMessageProfile()">
        ...
        <div class="robin-card-info robin-h-100 robin-flex robin-flex-column robin-flex-space-between robin-flex-1">
          <div class="robin-mt-6">
            <Text font-weight="normal" color="#000000" :font-size="14" :line-height="24" v-if="!conversation.is_group">
              {{ conversation.sender_token != $user_token ? conversation.sender_name : conversation.receiver_name }}
            </Text>

            <Text font-weight="normal" color="#000000" :font-size="14" :line-height="24" v-else>
              {{ conversation.name }}
            </Text>
          </div>

          <div class="robin-mt-6">
            <Text v-show="!conversation.is_group" as="p" font-weight="normal" color="#8D9091" :font-size="12" :line-height="14">{{ conversation.status == 'online' ? 'Online' : 'Offline' }}</Text>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="robin-options robin-ml-auto" @click="handleOpenPopUp($event, conversation.is_group ? 'popup-1' : 'popup-2')" v-show="!selectMessagesOpen">
      ...
      <div class="robin-popup-container" ref="popup-container" v-show="popUpState.opened && !selectMessagesOpen">
        ...
        <PersonalMessagePopOver ref="popup-2" v-show="!conversation.is_group" @view-profile="openMessageProfile()" />
      </div>
    </div>
  </header>
</template>
...
import GroupAvatar from '@/components/ChatList/GroupAvatar/GroupAvatar.vue'
import Avatar from '@/components/ChatList/Avatar/Avatar.vue'
import Text from '@/components/ChatList/Text/Text.vue'
import Button from '@/components/ChatList/Button/Button.vue'
