...
export default defineComponent({
  ...
  setup(props) {
    ...
    const getContactName = computed(() => {
      const index = robinUsers.value.findIndex(
        (user: any) => user.userToken === props.senderToken
      ) as number;
      const user = robinUsers.value[index] as any;

      if (user) {
        return user.userName && user.userName !== ''
          ? user.userName.trim().split(' ')
          : '';
      }
    ...
    const renderContactName = () => {
      ...
      if (getContactName.value.length > 1) {
        name =
          getContactName.value[0][0] +
          (getContactName.value[1].trim() !== ''
            ? getContactName.value[1][0]
            : '');
      }

      if (getContactName.value.length < 2) {
        name = getContactName.value[0][0];
      }
  ...
  render() {
    return (
      <div class="robin-avatar">
        {this.imgUrl == '' && this.getContactName == '' ? (
          <svg
            width="50"
            height="50"
            viewBox="0 0 50 50"
        ...
        {this.imgUrl == '' && this.getContactName != '' ? (
          <div class="robin-default-profile">
