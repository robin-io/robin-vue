    ...
    <div class="robin-modal-container robin-flex">
      <div class="robin-inner-container robin-flex robin-flex-column">
        ...
        <div class="robin-search">
          <SearchBar placeholder="Search people..." @user-typing="searchConversation($event)" :loading="isLoading" />
        </div>
              ...
              <div class="robin-card robin-flex robin-flex-align-center" v-for="(item, conversationIndex) in conversation" :key="item._id" :data-testid="`conversation-${item._id}`">
                ...
                <div class="robin-card-info robin-h-100 robin-h-100 robin-flex robin-flex-align-center robin-pt-4 robin-pb-4 robin-flex-1">
                  <div class="robin-flex">
                    <Content :font-size="14" :line-height="18" v-if="!item.is_group">{{ item.sender_token != $user_token ? item.sender_name : item.receiver_name }}</Content>

                    <Content :font-size="14" :line-height="18" v-else>{{ item.name }}</Content>
                  </div>

                  <div class="robin-ml-auto">
                    <CheckBox :key="addIndexToCheckBoxState(conversationIndex, checkBoxKeyState)" @clicked="toggleCheckAction($event, item)" ref="checkbox-comp" />
                  </div>
                </div>
              </div>
            </div>
